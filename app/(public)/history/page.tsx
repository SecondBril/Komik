...
export default function HistoryPage() {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      if (supabase) {
        const {
          data: { credentials, session } = {} as any,
        } = await supabase.auth.getSession();

        const activeSession = session || credentials;
            ...
            <div className="flex items-center gap-2 self-start sm:self-auto">
              ...
              <div className="flex items-center bg-[#FAF7F0] p-1 rounded-2xl border-2 border-[#1A1A1A] shadow-sm">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black transition-all ${activeTab === 'all'
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black transition-all ${activeTab === 'grouped'
                      ? 'bg-[#2E7D6E] text-white shadow-[1.5px_1.5px_0px_#1A1A1A]'
                      : 'text-[#7A756D] hover:text-[#1A1A1A]'
                    }`}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 sm:gap-4">
              {historyItems.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`flex items-center justify-between p-3.5 rounded-2xl bg-white border-2 border-[#1A1A1A] shadow-[3px_3px_0px_#1A1A1A] hover:shadow-[4px_4px_0px_#1A1A1A] transition-all gap-3 group ${isItemDeleting ? 'opacity-50 pointer-events-none' : ''
                      }`}
                  >
                    <Link
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 sm:gap-4">
              {groupedComics.map((group) => {
                ...
                return (
                  <div
                    key={group.comic_id}
                    className={`flex items-center justify-between p-4 rounded-2xl bg-white border-2 border-[#1A1A1A] shadow-[3px_3px_0px_#1A1A1A] hover:shadow-[4px_4px_0px_#1A1A1A] transition-all gap-3 group ${isGroupDeleting ? 'opacity-50 pointer-events-none' : ''
                      }`}
                  >
